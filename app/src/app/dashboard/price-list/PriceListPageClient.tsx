...
// Build unitPrices from SERVICES for compatibility
const unitPrices = {
    video: SERVICES.filter(s => s.category === 'VIDEO'),
    reklam: SERVICES.filter(s => s.category === 'CONSULTING'),
    podcast: SERVICES.filter(s => s.id.startsWith('podcast')),
    foto: SERVICES.filter(s => s.category === 'STUDIO' || s.category === 'PHOTO'),
    tasarim: SERVICES.filter(s => s.category === 'DESIGN'),
};
